using System.Collections.ObjectModel;
using System.Windows.Input;
using CAR_SERVICE_EF_RIDER.Messages;
using CAR_SERVICE_EF_RIDER.Models;
using CAR_SERVICE_EF_RIDER.Services.Interfaces;
using GalaSoft.MvvmLight;
using GalaSoft.MvvmLight.Command;
using GalaSoft.MvvmLight.Messaging;
using Org.BouncyCastle.Math;

namespace CAR_SERVICE_EF_RIDER.ViewModels;

public class AdminUserControlViewModel : ViewModelBase
{
    private readonly INavigationService NavigationService;
    private readonly IMessagesService MessagesService;
    private readonly IMessenger Messenger;
    private readonly ILoggerService LoggerService;
    private MyDbContext DbContext { get; set; } = new MyDbContext();
    public ObservableCollection<User> Users { get; set; }
    private Admin ActiveAdmin { get; set; }
    public ICommand DeleteCommand { get; }
    
    
    public AdminUserControlViewModel(INavigationService navigationService,IMessenger messenger,IMessagesService messagesService,ILoggerService loggerService)
    {
        NavigationService = navigationService;
        MessagesService = messagesService;
        Messenger = messenger;
        LoggerService = loggerService;
        
        Users = new ObservableCollection<User>(DbContext.Users.ToList());
        
        Messenger.Register<UpdateInfoMessage>(this, message =>
        {
            Users = new ObservableCollection<User>(message.DbContext.Users.ToList());
        });
        
        Messenger.Register<ActiveUserMessage>(this, message =>
        {
                ActiveAdmin = message.Person as Admin;
        });
        
        DeleteCommand = new RelayCommand<User>(DeleteItem);

    }
    
    private void DeleteItem(object obj)
    {
        if (obj is User user && Users.Contains(user)) // ChatGPT Helped
        {
            Users.Remove(user);
            DbContext.Remove(user);
            DbContext.SaveChanges();
            MessagesService.UpdateInfoMessage();
            LoggerService.CreateLog($"Admin '{ActiveAdmin.Name}' deleted user '{user.Name}'",1);
        }

    }
}